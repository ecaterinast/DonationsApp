...
export class OrderService {
  ...
  index: number; //index of edited order to be removed from waiting orders list automstically
  deleteOrder: BehaviorSubject<number> = new BehaviorSubject(undefined);
  ...
  //delete order
  delete() {
    this.deleteOrder.next(this.index);
  ...
  updateStock(status: OrderStatus, products: OrderProduct[]) {
    var promise = new Promise((resolve, reject) => {
      ...
      products.forEach(product => {
        ...
        //this condition is true if there is no stock for product
        //for older version -- just testing 😄 
